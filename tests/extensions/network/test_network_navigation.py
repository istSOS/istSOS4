...
def test_network_to_datastreams_grouping_A(client, network_seed):
    ...
    assert sorted(entity_id(d) for d in doc["value"]) == sorted(
        network_seed.a_ds_ids
    )
...
def test_network_to_datastreams_grouping_B(client, network_seed):
    ...
    doc = client.nav(
        f"Networks({format_id(network_seed.net_b_id)})/Datastreams"
    )
...
def test_network_datastreams_ref(client, network_seed):
    ...
    r = client.get(
        f"Networks({format_id(network_seed.net_a_id)})/Datastreams/$ref"
    )
    assert r.status_code == 200, r.text
...
def test_datastream_exposes_network_navlink(client, network_seed):
    ...
    assert (
        doc["Network@iot.navigationLink"]
        .rstrip("/")
        .endswith(f"Datastreams({format_id(ds_id)})/Network")
    )
