...
def test_eq(client, seed):
    ...
    assert filtered_results(client, seed, "result eq 4") == expect(
        seed, lambda r: r == 4
    )
...
def test_ne(client, seed):
    ...
    assert filtered_results(client, seed, "result ne 4") == expect(
        seed, lambda r: r != 4
    )
...
def test_gt(client, seed):
    ...
    assert filtered_results(client, seed, "result gt 4") == expect(
        seed, lambda r: r > 4
    )
...
def test_ge(client, seed):
    ...
    assert filtered_results(client, seed, "result ge 4") == expect(
        seed, lambda r: r >= 4
    )
...
def test_lt(client, seed):
    ...
    assert filtered_results(client, seed, "result lt 5") == expect(
        seed, lambda r: r < 5
    )
...
def test_le(client, seed):
    ...
    assert filtered_results(client, seed, "result le 4") == expect(
        seed, lambda r: r <= 4
    )
...
def test_eq_time_instant(client, seed):
    ...
    assert [
        o["phenomenonTime"].replace("+00:00", "Z") for o in values(doc)
    ] == [t.replace("+00:00", "Z")]
...
def test_and(client, seed):
    ...
    assert filtered_results(
        client, seed, "result gt 3 and result lt 6"
...
def test_or(client, seed):
    ...
    assert filtered_results(
        client, seed, "result lt 4 or result gt 5"
...
def test_not(client, seed):
    ...
    assert filtered_results(client, seed, "not (result gt 4)") == expect(
        seed, lambda r: not (r > 4)
    )
...
def test_not_with_function(client, seed):
    ...
    # not(result ge 5) -> {3,4}
    assert filtered_results(client, seed, "not (result ge 5)") == expect(
        seed, lambda r: not (r >= 5)
    )
...
def test_precedence_parentheses(client, seed):
    ...
    no_paren = filtered_results(
        client, seed, "result gt 5 or result gt 3 and result lt 5"
    )
    paren = filtered_results(
        client, seed, "(result gt 5 or result gt 3) and result lt 5"
    )
    assert no_paren == expect(
        seed, lambda r: r > 5 or (r > 3 and r < 5)
    )  # {4,6}
...
def test_add(client, seed):
    ...
    assert filtered_results(client, seed, "result add 1 eq 5") == expect(
        seed, lambda r: r + 1 == 5
    )
...
def test_sub(client, seed):
    ...
    assert filtered_results(client, seed, "result sub 1 eq 4") == expect(
        seed, lambda r: r - 1 == 4
    )
...
def test_mul(client, seed):
    ...
    assert filtered_results(client, seed, "result mul 2 eq 8") == expect(
        seed, lambda r: r * 2 == 8
    )
...
def test_div(client, seed):
    ...
    assert filtered_results(client, seed, "result div 3 eq 2") == expect(
        seed, lambda r: r / 3 == 2
    )
    assert filtered_results(client, seed, "result div 2 eq 2") == expect(
        seed, lambda r: r / 2 == 2
    )
...
def test_mod(client, seed):
    ...
    assert filtered_results(client, seed, "result mod 2 eq 0") == expect(
        seed, lambda r: r % 2 == 0
    )
...
def test_round(client, seed):
    ...
    assert filtered_results(client, seed, "round(result) eq 4") == expect(
        seed, lambda r: round(r) == 4
    )


def test_floor(client, seed):
    """req/request-data/built-in-query-functions -- floor(): floor(4)=4 -> {4}."""
    assert filtered_results(client, seed, "floor(result) eq 4") == expect(
        seed, lambda r: r == 4
    )
...
def test_ceiling(client, seed):
    ...
    assert filtered_results(client, seed, "ceiling(result) eq 4") == expect(
        seed, lambda r: r == 4
    )
