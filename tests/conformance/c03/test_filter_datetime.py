...
def expect_time(seed, py_pred) -> list:
    pairs = sorted(
        zip(seed.all_results, seed.all_phenomenon_times),
...
def test_year(client, seed):
    ...
    assert time_filter(
        client, seed, "year(phenomenonTime) eq 2015"
    ) == all_results(seed)
...
def test_month(client, seed):
    ...
    assert time_filter(
        client, seed, "month(phenomenonTime) eq 3"
    ) == all_results(seed)
...
def test_day_discriminates(client, seed):
    ...
    assert time_filter(
        client, seed, "day(phenomenonTime) eq 3"
    ) == expect_time(seed, lambda d: d.day == 3)
    assert time_filter(
        client, seed, "day(phenomenonTime) ge 5"
...
def test_hour(client, seed):
    ...
    assert time_filter(
        client, seed, "hour(phenomenonTime) eq 0"
    ) == all_results(seed)
...
def test_minute(client, seed):
    ...
    assert time_filter(
        client, seed, "minute(phenomenonTime) eq 0"
    ) == all_results(seed)
...
def test_second(client, seed):
    ...
    assert time_filter(
        client, seed, "second(phenomenonTime) eq 0"
    ) == all_results(seed)
...
def test_fractionalseconds(client, seed):
    ...
    assert time_filter(
        client, seed, "fractionalseconds(phenomenonTime) eq 0"
    ) == all_results(seed)
...
def test_totaloffsetminutes(client, seed):
    ...
    assert time_filter(
        client, seed, "totaloffsetminutes(phenomenonTime) eq 0"
    ) == all_results(seed)
...
def test_date_function(client, seed):
    ...
    assert time_filter(
        client, seed, "date(phenomenonTime) eq date(phenomenonTime)"
    ) == all_results(seed)
...
def test_time_function(client, seed):
    ...
    """
    assert time_filter(
        client, seed, "time(phenomenonTime) eq time(phenomenonTime)"
    ) == all_results(seed)
...
def test_now(client, seed):
    ...
    assert time_filter(client, seed, "phenomenonTime lt now()") == all_results(
        seed
    )
...
def test_mindatetime(client, seed):
    ...
    assert time_filter(
        client, seed, "phenomenonTime gt mindatetime()"
    ) == all_results(seed)
...
def test_maxdatetime(client, seed):
    ...
    assert time_filter(
        client, seed, "phenomenonTime lt maxdatetime()"
    ) == all_results(seed)
...
def test_instant_gt_literal(client, seed):
    ...
    assert time_filter(
        client, seed, f"phenomenonTime gt {cut}"
...
def test_closed_interval(client, seed):
    ...
    assert time_filter(
        client, seed, f"phenomenonTime ge {lo} and phenomenonTime le {hi}"
