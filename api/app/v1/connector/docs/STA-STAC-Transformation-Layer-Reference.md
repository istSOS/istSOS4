...
- `OPEN_CATALOG_METADATA` (default `1`): controls public access to shallow STAC routes (`/stac`, `/stac/collections`, `/stac/{network_id}`, etc.). When set to `0`, unauthenticated requests return `401 Unauthorized` (`WWW-Authenticate: Bearer`).
...
- **Root catalog link omission:** During harvest time, `stac_transformer.py` omits child links and `network_ids` references for any network ID listed in `CATALOG_CLOSED_NETWORKS`. This ensures anonymous clients fetching the root catalog `/stac` cannot discover closed networks from response links.
