...
class TestConnectionPoolBypass:
    ...
    async def test_connection_leak_on_exception_after_connect(self):
        ...
        async def mock_connect(**kwargs):
            ...
            leaked_connections.append({"conn": mock_conn, "closed": close_called})
    ...
    async def test_multiple_logins_create_multiple_connections(self):
        ...
        with patch("asyncpg.connect", side_effect=mock_connect), \
            ...
            tasks = [
                oauth.authenticate_user(f"user{i}", "pass")
                for i in range(10)
            ]
            await asyncio.gather(*tasks)
        
        print(f"\n✗ BUG CONFIRMED: {len(connection_count)} connections created for 10 logins")
        print("  With connection pool, only a few connections would be reused.")
        
        assert len(connection_count) == 10, (
    ...
    async def test_poor_error_handling_for_connection_errors(self):
        ...
        for error_class, message in test_errors:
            ...
            with patch("asyncpg.connect", side_effect=mock_connect):
                try:
                    await oauth.authenticate_user("test", "pass")
                    error_types_handled.append((error_class.__name__, "no exception raised"))
                except HTTPException as e:
                    # Error properly handled - converted to HTTPException with 503
                    assert e.status_code == 503, f"Expected 503, got {e.status_code}"
                    error_types_handled.append((error_class.__name__, "handled as 503"))
                except error_class:
                    # Error propagated without handling (BAD - old buggy behavior)
                    error_types_handled.append((error_class.__name__, "unhandled - leaked as 500"))
        ...
        for error_name, status in error_types_handled:
            ...
            assert "handled as 503" in status, f"{error_name} not properly handled!"
...
class TestConnectionPoolUsageAfterFix:
    ...
    async def test_timeout_is_configured(self):
        ...
        assert connect_params[0]["timeout"] == 5.0, f"Expected 5.0s timeout, got {connect_params[0]['timeout']}"
        assert "command_timeout" in connect_params[0], "Command timeout not configured!"
    ...
    async def test_logging_for_missing_user_in_table(self):
        ...
        mock_pool = MagicMock()
        mock_pool_conn = AsyncMock()
        mock_pool_conn.fetchrow = AsyncMock(return_value=None)  # User not in table
