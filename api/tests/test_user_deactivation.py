...
def test_deactivate_does_not_touch_auditlog():
    ...
    async def _run():
        ...
        try:
            ...
            try:
                await set_role(connection, {"username": "administrator"})
...
def test_deactivated_user_cannot_authenticate_with_correct_password():
    """authenticate_user() on this branch authenticates directly against a
    real PostgreSQL login role (pg_authid) -- there is no application-layer
    password column check here, unlike the bcrypt-based authenticate_user()
    on later branches. So this test creates a genuine, synthetic PG login
    role scoped to this test alone (DROP ROLE for a role that actually
    exists works fine; it's only individual *application* users that never
    get one -- see delete/user.py's own module docstring), authenticates
    against it while deactivated, and confirms the deactivation check still
    blocks it even though PostgreSQL itself would accept the credentials.

    Commits (rather than the rollback-only pattern used elsewhere in this
    file) for the same cross-connection-visibility reason as the other
    live tests: authenticate_user() queries through the app's own
    get_pool() connection, separate from whatever connection this test
    used to insert the row.
    """

    async def _run():
        username = f"{_MARKER}_bob"
        pg_password = "CorrectHorseBattery1!"
        setup_conn = await _connect_or_skip()
        try:
            await set_role(setup_conn, {"username": "administrator"})
            await setup_conn.execute(
                f'CREATE ROLE "{username}" LOGIN PASSWORD \'{pg_password}\';'
            )
            await setup_conn.execute(
                """
                INSERT INTO sensorthings."User" (username, role, status)
        ...
        try:
            ...
            with pytest.raises(Exception) as exc_info:
                await authenticate_user(username, pg_password)
            assert getattr(exc_info.value, "status_code", None) == 401
        finally:
            await _cleanup_committed_user(username)
            drop_conn = await _connect_or_skip()
            try:
                await drop_conn.execute('SET ROLE "administrator";')
                await drop_conn.execute(f'DROP ROLE IF EXISTS "{username}";')
            finally:
...
def test_deactivated_user_existing_jwt_is_rejected_on_next_use():
    ...
    async def _run():
        ...
        try:
            ...
            try:
                await set_role(deactivate_conn, {"username": "administrator"})
