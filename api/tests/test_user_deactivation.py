...
def test_deactivated_user_cannot_authenticate_with_correct_password():
    ...
    password column check here. So this test creates a genuine, synthetic
    PG login role scoped to this test alone (DROP ROLE for a role that
    actually exists works fine; it's only individual *application* users
    that never get one -- see delete/user.py's own module docstring),
    authenticates against it while deactivated, and confirms the
    deactivation check still blocks it even though PostgreSQL itself would
