...
class Observation(Base):
    ...
    def _serialize_columns(self):
        ...
        if (
            "phenomenon_time" in serialized_data
            and self.phenomenon_time is not None
        ):
            serialized_data["phenomenonTime"] = (
                self.phenomenon_time.isoformat()
            )
        ...
        if "valid_time" in serialized_data and self.valid_time is not None:
            serialized_data["validTime"] = self._format_datetime_range(
                self.valid_time
            )
            ...
            if relationship not in inspect(self).unloaded:
                ...
                if related_obj is not None:
                    relationship_key = (
                        ...
                        if relationship != "featuresofinterest"
                        else "FeatureOfInterest"
                    )
