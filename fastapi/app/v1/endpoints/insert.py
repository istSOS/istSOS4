...
async def generate_feature_of_interest(payload, conn):
    ...
    if result:
        ...
        if location_data["gen_foi_id"] is None:
            foi_payload = {
                "name": location_data["name"],
                "description": location_data["description"],
                "encodingType": location_data["encodingType"],
                "feature": location_data["location"],
            ...
            update_query = f"""
                ...
                WHERE id = $2::bigint
            """
            await conn.execute(update_query, foi_id, location_data["id"])

            payload["featuresofinterest_id"] = foi_id
        else:
            payload["featuresofinterest_id"] = location_data["gen_foi_id"]
    else:
        raise ValueError(
            "Cannot generate FeatureOfInterest for Thing with no locations."
        )
