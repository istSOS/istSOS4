...
async def catch_all_get(request: Request, path_name: str, db: Session = Depends(get_db)):
    ...
    try:    
        ...
        data = {}
        if len(item_dicts) == 1 and result["single_result"] and not full_path.endswith("s"):
            data = item_dicts[0]
        ...
        if not data or (isinstance(data, Iterable) and "value" in data and len(data["value"]) == 0):
            return JSONResponse(
